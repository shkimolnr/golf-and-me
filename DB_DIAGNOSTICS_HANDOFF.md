...
- Production 배포와 Vercel Secret 등록은 수행하지 않았습니다. 2026-08-31 Production Supabase SQL Editor에서 migration을 실행했고 `app_diagnostics` 테이블 생성을 확인했습니다.
...
1. Vercel의 Development/Preview/Production 환경에 `SUPABASE_SERVICE_ROLE_KEY`를 Secret으로 등록합니다. `VITE_` 접두사를 붙이거나 저장소에 넣지 않습니다.
2. `/api/diagnostics`의 Preview 테스트 후, 보관 삭제 함수의 실제 스케줄러를 선택·활성화합니다.
